...
int main() {
	...
	normal_distribution<double> N_x_init(0, sigma_pos[0]); // GPS measurement uncertainty, x
	normal_distribution<double> N_y_init(0, sigma_pos[1]); // GPS measurement uncertainty, y
	normal_distribution<double> N_theta_init(0, sigma_pos[2]); // GPS measurement unceratinty, theta
	normal_distribution<double> N_obs_x(0, sigma_landmark[0]); // Landmark measurement uncertainty, x
	normal_distribution<double> N_obs_y(0, sigma_landmark[1]); // Landmark measurement uncertainty, y
	double n_x, n_y, n_theta, n_range, n_heading;


	// Read position data
	vector<control_s> position_meas; //velocity, yaw rate
	...
	vector<ground_truth> gt; // x pos, y pos, yaw
	...
	for (int i = 0; i < num_time_steps; ++i) {
		...
		if (!pf.initialized()) {
			n_x = N_x_init(gen); //Gaussian Sampling random normal distribution of x noise position
			n_y = N_y_init(gen); //Gaussian Sampling random normal distribution of y noise position
			n_theta = N_theta_init(gen); //Gaussian Sampling random normal distribution of noise theta position
			pf.init(gt[i].x + n_x, gt[i].y + n_y, gt[i].theta + n_theta, sigma_pos); //initialize particle id,x,y,theta,weight
		}
		...
		vector<LandmarkObs> noisy_observations; // Landmark
		LandmarkObs obs;
		for (int j = 0; j < observations.size(); ++j) {
			n_x = N_obs_x(gen); //Gaussian Sampling random normal distribution of x position
			n_y = N_obs_y(gen); //Gaussian Sampling random normal distribution of y position
			obs = observations[j];
			obs.x = obs.x + n_x;
			obs.y = obs.y + n_y;
        ...
        //		check landmark index later if something wrong with the value
